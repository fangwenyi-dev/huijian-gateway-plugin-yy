...
# 2026-09-15 现场修：「暂停播放音乐」动词+播放+音乐宾语复合形被旧整句锚定
# 表项挡死 → 掉兜底。三组播控放宽「X播放+宾语可选」；裸「暂停」「停止」
# 仍不收（窗户/设备语义让位钉，fast_path 负向前瞻同律）。
_CTRL: list[tuple[str, re.Pattern]] = [
    ("stop",   re.compile(r"^(停止播放(音乐|歌曲?|这首歌)?|停止音乐|关掉音乐|关闭音乐"
                          r"|音乐关掉?|音乐停(一下|下)?|把音乐停(了|一下)?|把音乐关了?"
                          r"|别放了|停(一下|下)?(音乐|播放|歌)|停音乐)$")),
    ("pause",  re.compile(r"^(暂停播放(音乐|歌曲?|这首歌)?|暂停(音乐|歌曲?|这首歌)(播放)?"
                          r"|暂停(一下|下)(音乐|播放)?|音乐暂停|把音乐暂停(一下)?)$")),
    ("resume", re.compile(r"^(继续播放(音乐|歌曲?|这首歌)?|继续放(音乐|歌)?|恢复播放"
                          r"|接着放(音乐|歌)?)$")),
