...
_SCENE_DEL_RE = re.compile(
    ...
    r"[」\"』]?\s*(?:给我)?删(?:掉|了|除)$"
    # v1.0.49：名词先行变体「晚安场景删了/把睡觉的场景给我删除」——X 在前
    # "场景"在后。负排比裸删/复数指代（那些/所有/刚才）：那些不落精准删除。
    r"|(?!.*(所有|全部|这些|那些|刚才|刚刚|家里))(?:把|将)?(?P<x3>[^，。;；「」\"』\s]{2,10}?)"
    r"(?:这个|那个)?(?:的)?(?:语音)?场景(?:这个|那个)?(?:给我)?\s*删(?:掉|了|除)[。！!]?$")
...
_AUTO_DEL_RE = re.compile(
    ...
    # v1.0.49：名词先行「回家自动化删了/把回家的自动化给我删除」
    r"|(?!.*(所有|全部|这些|那些|刚才|刚刚|家里))(?:把|将)?(?P<t3>[^，。;；「」\"』\s]{2,10}?)"
    r"(?:这个|那个)?(?:的)?(?:语音)?自动化(?:这个|那个)?(?:给我)?\s*删(?:掉|了|除)[。！!]?$")
...
def parse(text: str) -> Optional[dict[str, Any]]:
    ...
    if m:
        x = (m.group("x") or m.group("x2") or m.group("x3") or "").strip()
    ...
    if m:
        raw = (m.group("t") or m.group("t2") or m.group("t3") or "").strip()
