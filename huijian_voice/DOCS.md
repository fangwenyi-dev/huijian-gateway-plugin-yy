# 慧尖HA语音插件（huijian_voice）

慧尖设备的**本地语音服务器**：识别（STT）、理解（NLU）、执行（HA 意图）、播报（TTS）
全部在局域网内完成，断网可用。零必填配置——安装、启动、配对，三件事。

> 本加载项与「慧尖 LoRa 网关」加载项同仓不同镜像，独立升级，互不影响。
> 依赖（可选增强）：`huijian_ai` HA 集成——**由本加载项自动落盘**，你只需重启一次 HA。

## 它是什么 / 不是什么

- ✅ 局域网语音链路服务器：`huijian_ai` 集成作为消费端连 `ws://<HA>:8000/xiaozhi/v1/…`
  （慧尖卫星固件走「模式 B」= ESPHome API :6053 入 HA，由 HA 语音管线调度；
  matter-broker 形态卫星与小程序也可按端点直连该 WS——三通道对 LAN 开放）
- ✅ 三层裁决理解级联（v1.0.9）：**场景触发词**（用户契约，恒最高优先）>
  **慧尖独占意图**（窗户内倒/暂停、模式切换、属性调节、语音场景与自动化管理、
  实时上下文——定位：引擎表达不了的才归慧尖意图）> **Klar 一级确定性 NLU**
  （标准控制恒优先：grounded entity_id 直调 HA 服务，不依赖慧尖集成）>
  TextCNN/T0 字面表兜底 > 查询族 > LLM（用户配置才启用，没配=无视）。
  Klar 只接管标准控制族（execute 判定 + 白名单 + 置信 ≥0.80），引擎缺席/失败
  全程 fail-open；执行期慧尖意图与 Klar **互为降级**——集成还没加载时，
  慧尖意图句自动兜到 Klar 直调，灯照样真亮
- ✅ HA 意图执行器：标准意图（`打开客厅的灯`）由 Klar 直调 `homeassistant.turn_on`
  等服务（**无需安装集成**即可控制灯/空调/窗帘/锁/扫地机——窗帘/纱窗是标准
  cover 归 Klar）；慧尖意图（窗户/天窗/内倒窗/开合器、模式、属性调节、场景与
  自动化管理）由 huijian_ai 集成执行（需安装：boot 自动落盘 → 重启 HA Core →
  添加集成）
- ❌ 不是公网小智服务器的替代品（公网链路已退役；不提供账号/唤醒云服务）
- ❌ 不自带大模型（LLM 兜底默认关闭，可自配 OpenAI 兼容端点）

## 安装（3 步）

1. 加载项商店添加本仓库（GitHub 拉不动时改用 Gitee 镜像源
   `https://gitee.com/fangwenyi-dev/huijian-gateway-plugin-yy`，逐提交同步，
   二选一勿同加）→ 安装「慧尖HA语音插件」→ 启动。
2. 首启自动下载语音模型（约 1.3 GB；离线环境见「模型」页的 import 投放口）。
3. **重启一次 HA Core**（加载自动落盘的 huijian_ai 集成）。

固件/小程序端：选择「慧尖HA语音插件加载项」模式（或手填三条端点，管理页「设备配对」
可复制）。局域网免 token 即连；对安全有要求可在设置页开启强制 token。

## 硬件建议

| 平台 | STT | TTS | 内存 |
|---|---|---|---|
| x86 / N100 | RTF≈0.04 | ~1s/句 | ≥2 GB |
| 树莓派 4/5 (aarch64) | 可用 | 可用 | ≥2 GB，建议开「空闲卸载」 |
| 树莓派 3 | 偏慢 | 偏慢 | 不建议 |

## 常见问答

**Q：装了但设备不说话？** 看「状态」页：STT/TTS 未就绪→「模型」页下载；
HA 桥接不可达→检查 HA 重启过没有；「调试」页输入一句看级联轨迹。

**Q：哪些话能听懂？** 开关/调节（亮度·色温·风量·温度·窗帘开合）、窗户（含内倒）、
场景触发词、时间/温度/状态查询、语音点歌与播控（需配置播放端点，见下一条）。
未命中的默认播兜底话术；开 LLM 兜底后可扩展。

**Q：语音点歌/放音乐怎么用？** 三步：
① 在 HA「设置 → 加载项商店」安装 **Music Assistant**（下称 MA，官方加载项），
在 MA 里添加本地音乐目录（建议放 `/share/music`，MA 自动扫描入库）；
② 本插件管理页「音乐」卡填**播放端点**：MA 托管的音箱 `media_player` 实体
（第三方网络音箱直接可用；慧尖语音卫星需固件音乐批支持，见固件版本说明）；
③ 开说——「播放周杰伦的晴天」由 MA 智能检索（歌名/歌手/专辑词皆可）、
「暂停播放/继续播放/下一首/上一首/停止播放/暂停」播控、「现在放的是什么歌」
回读曲名歌手（卫星端点不报曲目时，按本插件的点歌记账回答）；配了
**区域→播放端点映射**后支持「在客厅放音乐」定向投放，未映射区域自动回退
默认端点并如实播报。点歌只报歌名/歌手，网址与媒体路径一律不接（防呆闸）。
在线收听=在 MA 里添加电台/播客源（无授权风险的公开流）。QQ 音乐/网易云等
平台可经 MA 插件接入，但**需要您自备该平台账号与会员**，平台曲库不在本
插件交付范围内。
**端点能力预期管理**：第三方 MA 托管音箱=全功能（点歌/暂停/切歌/电台/长曲）；
慧尖卫星在固件音乐批落地前只能播 ≤30 秒短音频，「整曲播完」需固件 P2a 批、
「卫星电台」需 P2b 批——P2a 前的卫星形态整曲点播有可感的下载等待（话术会
提示"等一小会儿"），秒开档音箱可在设置里关掉该提示。经 MA 推给慧尖卫星的
转码档请钉 **MP3、单声道、≤128kbps**（MA 播放器设置里配）。

**Q：为什么"空调调到26度"不执行？** 缺区域信息时按守卫拒执行（防误伤同型号多空调），
请说「客厅空调调到26度」，或开启 LLM 兜底让它追问。

**Q：音色能换吗？** 设置页 12 个内置音色（默认 小北 sid45），云端 TTS 亦可配。

**Q：想接在线大模型 / 云端 STT·TTS？** 设置页三张卡各有「平台预设」下拉：选中平台即自动
填好 Base URL/模型名/音色，**只需粘贴 API Key → 保存**。预设收录纪律 = 仅标准
OpenAI 兼容端点：

- **LLM 兜底（12 家）**：DeepSeek、阿里百炼、火山方舟（豆包）、智谱 GLM
  （glm-4-flash 免费）、Kimi、硅基流动、讯飞星火、Gemini、OpenAI、302 类代理网关、
  局域网 Ollama（免 Key，改 IP 即用）。
- **云端 STT（5 家）**：百炼 Qwen3-ASR、硅基流动 SenseVoice、Groq Whisper、
  OpenAI Whisper、302.AI；云端失败自动回落本地 Paraformer。
- **云端 TTS（3 家）**：硅基流动 CosyVoice2（8 种中文预置音色 anna/alex 等）、
  OpenAI TTS、302.AI；平台回 wav 自动解包取实际采样率，回 mp3/opus 会明确
  提示改输出格式。

每个预设下方直接标注 Key 申请入口；平台改模型名属常态——直接在模型名框改即可，
无需等插件更新。讯飞/火山/腾讯/百度的**语音**接口是私有协议，OpenAI 兼容通道
接不了，故不收（它们的 LLM 是标准兼容，可选）。

**Q：完全离线？** 用 `/data/models/import/` 投放模型 + 关闭自动下载即可，无任何出网需求。

## 排障

- **Supervisor 日志刷 `supervisor.store.git … unexpected eof while reading` /
  `Could not reload repository … StoreGitError`**：GitHub 被你的网络侧干扰
  （SNI 重置/DNS 污染），商店拉不到仓库**清单**——**不是加载项故障**：已装
  加载项照常运行（镜像走阿里云 ACR、运行纯局域网，均不依赖 GitHub），只是
  看不到新版本、商店不刷新。处置按序：
  ① 慧尖系仓库换 Gitee 镜像源（逐提交同步内容一致）：「仓库」里删 GitHub
  源、加 `https://gitee.com/fangwenyi-dev/huijian-gateway-plugin-yy`
  （网关仓库为 …/ha-gateway-plugin）——**二选一，勿同加**（同 slug 会重复）；
  ② 给 HA 主机设静态 DNS（223.5.5.5 / 119.29.29.29）可缓解 DNS 污染型失败；
  ③ 第三方仓库（如 AlexxIT、songloft）无国内镜像可换：可暂时从「仓库」列表
  移除消除刷屏，需要更新时再加回；Supervisor 本身会自动周期重试。
  注意：**当前版 Supervisor 已无「互联网代理」配置项**（源码实证），网上老
  教程的 `ha supervisor options --proxy-url` 不再适用，勿照做。
- **商店里看不到「慧尖HA语音插件」卡片**：先看 Supervisor 日志（系统→诊断）里有无
  `Can't read .../config.yaml`。加载项仓库清单是**整份校验失败即整体静默跳过**
  （无前端报错）——历史上网关加载项就因一行臆造的 schema 语法踩过此坑，
  本仓有钉桩防线；若你添加的是 fork/改版仓库，请以本仓为准。
- **安装一直停在"下载"（Supervisor 日志停在 `Downloading docker image`）**：
  本加载项镜像自 v1.0.1 起走**自有阿里云 Registry 国内直连**
  （`crpi-…cn-shanghai.personal.cr.aliyuncs.com`，单仓多架构自动匹配），
  正常分钟级完成。若仍卡住（如 ACR 服务波动），处置三步：
  ① 等 10~30 分钟后**卸载再重装**（重装即重新拉取）；
  ② 仍慢则「设置→加载项→仓库」移除本仓库重新添加后再装；
  ③ 灾备换源：把仓库文件中 `huijian_voice/config.yaml` 的 image 整行临时改为
  `image: ghcr.io/fangwenyi-dev/{arch}-huijian-voice`（同镜像官方源，
  需你的网络可达 GitHub Registry），保存后检查更新再安装。
- **刚出新版后点「更新」报 `An unknown error occurred with app … Check
  Supervisor logs`**：新版本号进入商店 git 后，镜像还要 ~15 分钟才经 CI
  （构建→e2e→推送 ACR）出厂；点太快 Supervisor 拉到不存在的镜像 tag，
  就报这个笼统错误。**等十几分钟再点一次「更新」即可**，不是加载项故障。
  发布方自检镜像是否已就位可跑仓库 `scripts/verify_release.py`
  （ACR/ghcr 双源 manifest+blob 探活，退出码 0 = GO）。
- **按唤醒键无反应、设备日志 `HA did not answer` 或 `code=validation-error`**：
  ① v1.0.16+ 引擎条目缺失会在设备装配时自动补建、v1.0.17+「慧尖语音」管道
  （STT/TTS/对话引擎接线）也会自动配好——重启 HA 即自愈；旧版本需确认
  「设置→语音助手」里慧尖管道存在且语音转文本/文本转语音已选慧尖引擎；
  ② 固件需 ≥ v2.1.10（v2.1.9 及以前存在按钮会话 0ms 假超时缺陷，
  现象即"发了请求立刻自己拆会话"）。
- **装好后设备连不上**：确认设备与 HA 主机同网段（本加载项**纯局域网**，
  不提供公网通道）；管理页「配对」区复制带 token 的完整端点给设备。
- **首次说话延迟高**：首启要下载模型（约 450MB，管理页「模型」区看进度）；
  开了「空闲卸载」后每段空闲的首句会多 ~2s 重载时间，省电与响应二选一。

## 限制（诚实清单）

- 「保存为场景 / 创建·修改自动化」语音路径 M1 实装（当前引导到 HA 界面）；
- 音乐：点歌/播控/正在播放查询已支持（需 MA + 播放端点，见常见问答）；慧尖语音
  卫星本体的流式播放与固件侧播控随固件音乐批演进，旧固件卫星上「暂停/下一首」
  等指令以端点实际能力为准；平台音乐（QQ/网易云等）需用户自备会员，不随插件交付；
- 识别语种：中文 + 中英混说（Paraformer 双语流式），纯英文长句质量一般；
- 唤醒词不在本加载项职责内（固件端本地唤醒）。

## 端口占用

| 端口 | 用途 | 暴露范围 |
|---|---|---|
| 8000 | 小智协议 WS（stt/tts/llm 三通道）+ `GET /discover` 端点自描述 | 局域网（设备连） |
| 8001 | 管理页（Ingress） | 仅 HA/Supervisor（源 IP 白名单） |
| 8002 | 管理 API | 仅容器回环 |

`/discover` 为无凭据 JSON（小程序 mDNS 读不到 TXT 的替代发现面）：只回三条
端点路径、音频参数与是否强制 token，**永不回 token**。
