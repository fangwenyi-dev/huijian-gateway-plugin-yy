...
| 插件 | 说明 | 形态 |
|---|---|---|
| **慧尖语音助手**（huijian_voice） | STT/TTS/NLU 服务器 + Ingress 管理页 + huijian_ai 集成自动落盘 | Debian 容器（amd64 / aarch64） |

## 安装方法

1. HA「设置 → 加载项 → 商店 → ⋮ → 仓库」添加本仓库 URL；
2. 商店安装「慧尖语音助手」，启动加载项（首启自动下载模型，约 450MB，
...
- 测试：`cd huijian_voice && python -m pytest tests -q`（111 项钉桩）
